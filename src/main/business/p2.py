...
configuration.api_key['tokenAuth'] = 'KJzghGklAdrMQD3oqGvp1GLfrle9WhSxgA4h5EFE6NxmNAexS9XdFA'
...
with MergeAccountingClient.ApiClient(configuration) as api_client:
    ...
    x_account_token = "6oKJdoXds9L_yW1YMVQwWUUiKJNsShAdHjEX9zq1y4bO3MOsKRH0ZA" # str | Token identifying the end user.
    created_after = parse('1970-01-01T00:00:00.00Z') # datetime | If provided, will only return objects created after this datetime. (optional)
    created_before = parse('1999-01-01T00:00:00.00Z') # datetime | If provided, will only return objects created before this datetime. (optional)
    ...
    remote_id = "81" # str, none_type | The API provider's ID for the given object. (optional)

    # example passing only required values which don't have defaults set
    try:
        api_response = api_instance.transactions_list(x_account_token, cursor=cursor, remote_id=remote_id, created_after=created_after)
        pprint(api_response)
